...
async def receive_sensor_data(request: Request):
    ...
    if severity in ["WARNING", "CRITICAL"]:
        ...
        gate = getattr(request.app.state, "alert_gate", None)
        ...
        if hasattr(request.app.state, "publisher") and request.app.state.publisher is not None:
            request.app.state.publisher.publish_alert(node_id, severity, alert_reason)
        logger.warning("Alert generated for Node %s: %s (%s)", node_id, severity, alert_reason)
